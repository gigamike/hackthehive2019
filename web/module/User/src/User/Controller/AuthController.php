...
class AuthController extends AbstractActionController
{
    ...
    {
      $volunteerId = (int)$this->params('id');

      $authService = $this->serviceLocator->get('auth_service');
      if ($authService->getIdentity()!=null) {
        if($authService->getIdentity()->role == 'admin'){
          return $this->redirect()->toRoute('admin');
        }else if($authService->getIdentity()->role == 'volunteer'){
          return $this->redirect()->toRoute('volunteer');
        }else if($authService->getIdentity()->role == 'ofw'){
          ...
          if($result->isValid()) {
            ...
            if ($authService->getIdentity()!=null) {
              if($authService->getIdentity()->role == 'admin'){
                ...
                /*
                if ($volunteerId) {
                  $product = $this->getProductMapper()->getProduct($volunteerId);
                  if($product){
